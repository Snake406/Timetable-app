...
function loadLessons(): LessonMap {
  try {
    const saved = localStorage.getItem("timetableLessons2");
...
const [holdTimeout, setHoldTimeout] = useState<ReturnType<typeof setTimeout> | null>(null);
  ...
  const [title, setTitle] = useState(localStorage.getItem("timetableTitle2") || "Title is editable");
  ...
  // ── Persistence ────────────────────────────────────────────────────────────
  useEffect(() => { localStorage.setItem("timetableLessons2", JSON.stringify(lessons)); }, [lessons]);
  useEffect(() => { localStorage.setItem("timetableTitle2", title); }, [title]);
  useEffect(() => {
    const img = localStorage.getItem("timetableImage2");
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      localStorage.setItem("timetableImage2", base64);
    };
            ...
            <IonInput
              ...
              onBlur={() => {
                ...
                if (!trimmed) {
                  ...
                  setTitle(localStorage.getItem("timetableTitle2") || "Page 1");
                } else {
                  setTitle(trimmed);
                }
                setIsEditing(false);
