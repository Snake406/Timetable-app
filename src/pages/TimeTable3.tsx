...
function loadLessons(): LessonMap {
  try {
    const saved = localStorage.getItem("timetableLessons3");
...
const [holdTimeout, setHoldTimeout] = useState<ReturnType<typeof setTimeout> | null>(null);
  ...
  const [title, setTitle] = useState(localStorage.getItem("timetableTitle3") || "Title is editable");
  ...
  // ── Persistence ────────────────────────────────────────────────────────────
  useEffect(() => { localStorage.setItem("timetableLessons3", JSON.stringify(lessons)); }, [lessons]);
  useEffect(() => { localStorage.setItem("timetableTitle3", title); }, [title]);
  useEffect(() => {
    const img = localStorage.getItem("timetableImage3");
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      localStorage.setItem("timetableImage3", base64);
    };
            ...
            <IonInput
              ...
              onBlur={() => {
                ...
                if (!trimmed) {
                  ...
                  setTitle(localStorage.getItem("timetableTitle3") || "Page 1");
                } else {
                  setTitle(trimmed);
                }
                setIsEditing(false);
