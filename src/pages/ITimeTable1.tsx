...
const ITimeTable: React.FC = () => {
  ...
  const [pageLabel, setPageLabel] = useState("Image Timetable 1");

  // ── Persist image ──────────────────────────────────────────────────────────

  useEffect(() => {
    const savedImg = localStorage.getItem("ItimetableImage");
    if (savedImg) setSelectedImageI(savedImg);

    const savedLessons = localStorage.getItem("ItimetableLessons");
    const savedLabel = localStorage.getItem("IcachedPageLabel");
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      localStorage.setItem("ItimetableImage", base64);
    };
  ...
  const fetchLessons = async (pageNumber: number) => {
    ...
    for (const day of weekdays) {
      ...
      setLessons(map);
      localStorage.setItem("ItimetableLessons", JSON.stringify(map));
      localStorage.setItem("IcachedPageLabel", pageKey);
    } catch (err) {
